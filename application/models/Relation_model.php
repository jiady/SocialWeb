...
class Relation_model extends CI_model{
    ...
    function addBlackList($id, $blackid) {
        ...
        $update="UPDATE friendrequest SET accepted=true WHERE from_uid=".$this->db->escape($blackid)." AND to_uid=".$this->db->escape($id);
    ...
    function sendFriendRequest($id, $to_id, $reason="Hello") {
    	$query="DELETE FROM blacklist WHERE from_uid=".$this->db->escape($id)." AND to_uid=".$this->db->escape($to_id);
    	$this->db->query($query); 
